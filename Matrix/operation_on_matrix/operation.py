def classic_multiplication(A:list, B:list):
    C = [0, 0]
    B_dash = [] # another matrix to store the outer layer of matrix B
    first_matrix_length = len(A)
    second_matrix_length = len(B)

    # check outer layer for the first matrix
    first_outer = A[0] # outer of the left-side is the first row
    
    # check outer layer for second matrix
    for i in range(second_matrix_length):
        B_dash.append(B[i][-1]) # perform operation to find the last node in the matrix for each column
    second_outer = B_dash

    if len(first_outer) == len(second_outer):
        # make an empty matrix C with the right size to fill in the transformation
        C = [[0 for _ in range(len(first_outer))] for _ in range(len(second_outer))]
